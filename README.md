...
For a reconciled `adopt-existing` document, later source syncs preserve the ABCM-owned target frontmatter, copy an explicitly present Git `tags` field, and replace the Markdown body including inline tags. Ordinary non-reconciled mirrors remain byte-exact. Source and materialized target checksums stay distinct in provenance.

An authorized ABCM Sync for Obsidian device acts as the operator for its own checksum-bound edit of the current accepted ADR/RFC. The server archives the exact previous revision under the sibling `revisions/<artifactId>.md` path, assigns the edited document a new immutable artifact identity and keeps the visible vault path on the new accepted head. The receipt returns the canonical server checksum, and the plugin replaces its local edit with those checksum-verified canonical bytes before acknowledging the outbox. Direct MCP/file writes, stale base checksums, and devices without write capability remain blocked.
