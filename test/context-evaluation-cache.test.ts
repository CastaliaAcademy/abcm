...
describe("versioned context cache and proposal-only feedback", () => {
  test("отличает cold, warm и stale, не смешивая principal, access, MapRevision и policy", async () => {
    ...
    try {
      ...
      expect(first.connectedSkills).toContainEqual(expect.objectContaining({ skillId: "cache-skill", body: expect.stringContaining("SKILL_BODY_SENTINEL_V1") }));
      expect(second.connectedSkills).toEqual(first.connectedSkills);
      ...
      expect(rebuilt.connectedSkills[0]?.body).toContain("SKILL_BODY_SENTINEL_V2");
      ...
      expect(JSON.stringify(cachedPayloads)).not.toContain("SKILL_BODY_SENTINEL");
      database.close();
    } finally {
      await runtime.close();
    }
