...
export class SqliteScopeMapStore implements ScopeMapStore, ContextFingerprintCatalog, ContextBuildCacheCatalog {
  ...
  #migrate(): void {
    this.#database.transaction(() => {
      ...
      if (currentVersion < 12) {
        // Centralized evaluation/feedback persistence was removed. Context fingerprints and cache remain operational data.
        this.#database.run("DROP INDEX IF EXISTS business_evaluation_dataset_idx");
        this.#database.run("DROP TABLE IF EXISTS business_evaluation_receipts");
        this.#database.run("DROP TABLE IF EXISTS context_feedback_proposals");
        this.#database.run("DROP TABLE IF EXISTS context_outcomes");
      }
      this.#database.run(
