...
export const ABCM_AGENT_INSTRUCTIONS_VERSION = "1.26.0" as const;
...
- Пакет ссылок (LinkPackage): виртуальная проекция текущей ScopeMap для одного нормализованного тега. Теги берутся из Markdown frontmatter tags и явных inline-тегов #tag; создание, изменение, перемещение и удаление файлов автоматически меняет пакет при следующей индексации. Один документ может иметь не более 64 уникальных нормализованных тегов, каждый длиной до 256 символов, общим объёмом до 4096 UTF-8 байт. packageId стабилен для workspace и тега, а packageDigest фиксирует MapRevision и текущий упорядоченный набор доступных documentId. Пакет не является отдельной записью, не публикуется и не передаёт права: при каждом build сервер проверяет workspace запроса и повторно авторизует каждый документ для текущего principal. Cross-workspace ссылки запрещены. Результат build содержит disclosure-safe members со статусами selected, selector_mismatch, budget_omitted или lifecycle_omitted только для доступных principal участников.
...
- Amendment: единственный путь изменить нормативный смысл или relations принятого ADR/RFC. Draft объявляет lineageId, amends, baseArtifactId, baseChecksum и expectedLineageHead. Preview связывает draft checksum, base checksum, head и MapRevision. Затем отдельная operator identity через защищённый REST-маршрут выпускает короткоживущий одноразовый approvalReceiptId; агент не может сам сформировать receipt. Авторизованное write-capable устройство ABCM Sync for Obsidian считается оператором только для собственного checksum-bound изменения current accepted head: сервер сохраняет предыдущую точную редакцию в sibling-каталоге revisions, публикует новый artifactId по прежнему видимому пути и возвращает canonical checksum. Acceptance создаёт новую accepted revision с supersedes и не изменяет байты архивированной редакции. Два конкурирующих amendment не могут одновременно стать head; direct MCP write, stale base checksum и read-only либо отозванная интеграция блокируются.
...
- context.build_from_link_package: проверить workspace пакета, повторно авторизовать consumer principal и построить ContextBundle через стандартный ContextBuilder; поле members объясняет результат каждого доступного участника без раскрытия закрытых documentId.
...
Не смешивайте MCP connector и ABCM Sync for Obsidian. MCP является интерфейсом агента к workspace/project. Двусторонний Obsidian-плагин является отдельным пользовательским клиентом scoped REST sync API и не добавляет pairing, device, cursor или conflict операции в MCP manifest. Оба клиента используют одну server-assigned границу workspaceId/projectId, но имеют разные credentials и состояние.

Структура реплики плагина: установленный код находится в <Vault>/.obsidian/plugins/abcm-sync и никогда не синхронизируется; выбранная <vaultFolder> соответствует одному project prefix; видимый <vaultFolder>/_ABCM Conflicts содержит конфликтные копии и также исключён из синхронизации. Device credential хранится только в Obsidian SecretStorage.
...
Если внешний legacy-корпус уже был преобразован в каноническую архитектуру ABCM, включите fail-closed reconciliation с unmappedPolicy=conflict. Workspace-owned manifest обязан закрепить каждый sourcePath за единственным canonical targetPath и обеими контрольными суммами. disposition=adopt-existing при первой привязке сохраняет канонические байты и фиксирует provenance; последующий sync сохраняет ABCM-owned frontmatter, берёт из Git явно объявленное поле tags и заменяет Markdown body вместе с inline-тегами. Отсутствующая запись, повторный target, изменившийся source/target либо отсутствующий target возвращают DOCUMENTATION_RECONCILIATION_REQUIRED или DOCUMENTATION_RECONCILIATION_STALE и блокируют apply, sync и cutover. Нельзя заменять такой conflict записью в fallback import subtree.
...
Правильно: авторизованному write-capable Obsidian-устройству изменить current accepted ADR/RFC по его стабильному vault path, дождаться canonical checksum в receipt и принять проверенные серверные байты. Предыдущая редакция остаётся доступной в sibling-каталоге revisions/<artifactId>.md.

Контрпример: повторить изменение с устаревшим base checksum, использовать read-only устройство или попытаться переписать accepted ADR/RFC через MCP. Такие операции обязаны завершиться конфликтом либо отказом доступа без нового lineage head.
...
- MCP initialize.serverInfo.version — версия пакета/сервера, а версия этой инструкции — версия эксплуатационного контракта. После изменения любой из них или серверной конфигурации переподключите connector/tunnel и заново получите tools/list. Полный настроенный контур сервера 0.1.7 с инструкцией 1.26.0 содержит 41 операцию: два versioned v4 entrypoint предотвращают молчаливое удаление новых полей connector-кэшем; четыре documentation-source операции присутствуют только при настроенных оператором каталогах. Централизованные outcome, feedback, business-evaluation и task-success операции намеренно отсутствуют.
